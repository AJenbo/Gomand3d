/*
$Header:   H:/pvcs/pvcsproj/gomand3d.prj/D3DAPP.C_V   1.13   15 May 1997 17:46:06   Jeremiah  $

$Log:   H:/pvcs/pvcsproj/gomand3d.prj/D3DAPP.C_V  $
 * 
 *    Rev 1.13   15 May 1997 17:46:06   Jeremiah
 * Changed default sfx and midi levels.
 * 
 *    Rev 1.12   May 07 1997 16:55:38   JEREMIAH
 * Separated stretch flags into separate 
 * structure.  Added bWillStretch.  Allowed
 * stretch to be selectable from window size
 * menu.
 * 
 *    Rev 1.11   28 Apr 1997 17:27:24   JEREMIAH
 * Removed hardcoding of bCanStretch.
 * Check with QuickD3DCheck().
 * 
 *    Rev 1.10   28 Apr 1997 10:14:02   TIM
 * Made mods for s3 stretch
 * 
 *    Rev 1.9   12 Mar 1997 10:08:20   JEREMIAH
 * Added call to set the transparency style
 * on d3d startup.
 * 
 *    Rev 1.8   25 Feb 1997 17:11:06   BRUCE
 * Put time totals into GameStates.
 * 
 * 
 *    Rev 1.7   14 Feb 1997 19:27:40   BRUCE
 ...
 * changed LevelSoFar to GameStates.CurrentLevel
 * changed PlayMode to GameStates.IsTraining
 * 
 *    Rev 1.6   06 Feb 1997 08:14:34   BRUCE
 * took control of whichRobot, 
 * removed uninteresting warning.
 * 
 *    Rev 1.5   09 Jan 1997 14:59:04   BO
 * Removed VMode variable from the file
 * 
 *    Rev 1.4   17 Dec 1996 10:59:20   JEREMIAH
 * Changed by Bo.  Added support for updating
 * game windows smaller than full-screen.
 * 
 *    Rev 1.3   03 Dec 1996 11:57:18   JEREMIAH
 * Modified CHECKSURF() macro to include
 * more error checking.
 * 
 *    Rev 1.2   Nov 01 1996 11:53:58   JEREMIAH
 * Changed by Bo.  Removed extra 
 * BYDumpInfo calls.  Modified 
 * D3DAppCheckForLostSurfaces() to handle
 * ALT-TAB problem.
 * 
 *    Rev 1.1   Oct 24 1996 14:43:48   JEREMIAH
 * Changed by Bo.  Added code to check and
 * restore 2D surfaces.
 */
...
int VM_H3D;
int VM_BPP3D;
BOOL VM_FULLSCREEN=TRUE;
//int level = 5;
//int OldLevel, LevelSoFar, 
int whichavi;
//BOOL   AutoDemoFlag = FALSE;
BOOL   AutoDemoNext = TRUE;
//short  PlayMode = SYS_TRAINING;// SYS_REAL_GAME;
BOOL   QuitFlag=FALSE;

//unsigned short RobotStatus[5] = {1,1,1,1,1};
short SFXValue=80;
short MidValue=66;
//unsigned char Hours = 0;
//unsigned char Minutes = 12;
//unsigned char Seconds = 24;
short KnobLength;
short BarLength;
short BarStartP;

BOOL DDmode0, DDmode1, DDmode2, DDmode3, DDtext;
WORD nTimer; 

FunctionCall BYRender;
...
// Tim Stretch:  structure of Stretch info 

sStretchInfo sStretch;
...
//	 char ftp[127];
    ...
    *D3DApp = &d3dappi;
	...
	if (VM_FULLSCREEN) {
		mode = BYGetRealMode(&VM_W3D, &VM_H3D, &VM_BPP3D);
	...
	if (mode == D3DAPP_USEWINDOW) 
	{
        ...
        ATTEMPT(BYCreate3Overlays((short)VM_W, 1));
 ...
 * D3DAppFullscreen
 */
BOOL D3DAppFullscreen(int mode, int owidth, int oheight)
{
    int w, h, bpp;
	...
	ATTEMPT(BYCreate3Overlays((short)owidth, 1));
...
D3DAppChangeDriver(int driver, DWORD flags)
{
    ...
    if (mode == D3DAPP_USEWINDOW) {
         ...
         * same function.
         */
        ATTEMPT(D3DAppFullscreen(mode, VM_W3D, VM_H3D));
        return TRUE;
    }
    ...
    switch(message) {
        ...
        case WM_ACTIVATEAPP:
            d3dappi.bAppActive = (BOOL)wParam;
				if (Page!= pg_SHOW_GAME_SCREEN || 
					BYRender == Render3DWorld)
				   bBYRenderingIsOK = d3dappi.bAppActive;

				if (d3dappi.bAppActive)
					OutputDebugString("d3dappi.bAppActive=TRUE\n");
				else
					OutputDebugString("d3dappi.bAppActive=FALSE\n");
            break;
        case WM_SETCURSOR:
            /*
...
D3DAppAddTexture(const char* imagefile)
{
    ...
    if (d3dappi.ThisDriver.bDoesTextures) {
        ATTEMPT(D3DAppILoadTextureSurf(d3dappi.NumTextures));
...
D3DAppSwapTextures()
{
     ...
     */
    for (i = 0; i < d3dappi.NumTextures - 1; i++) {
 ...
 */
#define CHECKSURF(x) if (x && x->lpVtbl) {                                               \
...
D3DAppCheckForLostSurfaces(void)
{
   ...
   CHECKSURF(d3dappi.lpBackBuffer);
   	//Tim Stretch:
	if(sStretch.bStretch) CHECKSURF(d3dappi.lpBackBuffer2);
   CHECKSURF(d3dappi.lpZBuffer);
   CHECKSURF(lpDDSOverlay[0]);
   CHECKSURF(lpDDSOverlay[1]);
   CHECKSURF(lpDDSOverlay[2]);
	...
	if (b) 
	{
		...
		if (VSize!=4 && !bUseDebug && BYRender==Render3DWorld)
		{
		// There is a unknow bug here. In 320 mode, BYBlitOnToP doesn't
		// work.  Even I use flip to flip back buffer and then call
		// BYDrawOverLay(1,TRUE) again, the flip action also looks no
		// effect.  
			...
			myglobs.view->lpVtbl->ForceUpdate(myglobs.view,
					                    GWinSize[vmd][VSize-1].left, 
					                    GWinSize[vmd][VSize-1].top, 
											  GWinSize[vmd][VSize-1].right,
                                   GWinSize[vmd][VSize-1].bottom);
			myglobs.dev->lpVtbl->Update(myglobs.dev);
		} 
	} 
...
BOOL BYCreateDDForWrapper(HWND hwnd)
{
	HDC hdc;
	int i, mode, w, h;
//	char ftp[127];
	...
	if (LastError != DD_OK) 
	{
		BYDumpInfo("DirectDrawCreate failed.");
		return FALSE;
	}
	...
	// Remember current windows mode
	ATTEMPT(D3DAppIRememberWindowsMode());
	// set cooperative level = exclusive + allow modex
   ATTEMPT(D3DAppISetCoopLevel(hwnd, TRUE));
	// enumerate display modes
   ATTEMPT(D3DAppIEnumDisplayModes());
	mode = BYGetRealMode(&VM_W, &VM_H, &VM_BPP);
	if (mode<0)
	{
		BYDumpInfo("Display driver doesn't support this mode!");
		return FALSE;
	}
	d3dappi.bFullscreen = TRUE;
	D3DAppIGetClientWin(hwnd);
...
// This function only works for exclusive mode
BOOL BYCreateDDDFor3DGame(DWORD flags, HWND hwnd,
                          BOOL(*DeviceCreateCallback)(int, int,
                                                      LPDIRECT3DVIEWPORT*,
                                                      LPVOID),
                          LPVOID lpCreateContext,
...
//	char ftp[127];
	...
	//Tim Stretch: set up stretch foe 3d portions
	if (sStretch.bCanStretch && sStretch.bWillStretch)
		// do stretch in game
		sStretch.bStretch = TRUE;


	// Create DirectDraw & remeber the Windows display mode
	ATTEMPT(D3DAppICreateDD(0L));
	ATTEMPT(D3DAppIRememberWindowsMode());

	// set cooperative level = exclusive mode + allow modex
	ATTEMPT(D3DAppISetCoopLevel(hwnd, TRUE));

	// enumerate video modes (only width=640 or 320 are accepted
	ATTEMPT(D3DAppIEnumDisplayModes());
	...
	*D3DApp = &d3dappi;
	...
	// set display mode, create front, back buffer, 3 overlays
	ATTEMPT(D3DAppISetDisplayMode(w, h, d3dappi.Mode[mode].bpp));
	d3dappi.CurrMode = mode;
	ATTEMPT(D3DAppICreateBuffers(hwnd, w, h, d3dappi.Mode[mode].bpp, TRUE));
	ATTEMPT(BYCreate3Overlays((short)VM_W3D, 1));
   // If the front buffer is palettized, load our palette
	ATTEMPT(D3DAppICheckForPalettized());

   ATTEMPT(D3DAppICreateZBuffer(w, h, driver));
   // Create the D3D device, call the device create
   // callback and set a default render state
	ATTEMPT(D3DAppICreateDevice(driver));
	ATTEMPT(D3DAppIFilterDisplayModes(driver));  /* bThisDriverCanDo flags */
	ATTEMPT(D3DAppICallDeviceCreateCallback(w, h));
	ATTEMPT(D3DAppISetRenderState());
   ATTEMPT(SetTransparencyStyle());
   // Initialize dirty rectangle information
	D3DAppIValidateDirtyRects();
...
exit_with_error:
	 OutputDebugString("Restore 2D surfaces are failed!\n");
    return FALSE;
}
